const { isMatch, isBefore, isEqual, lastDayOfMonth, format, addMonths, differenceInMonths } = require("date-fns");
...
const handleDate = {
    ...
    getListOfMonths(data_inicio, data_fim) {
        ...
        const list_months = [];
        const amount_months = differenceInMonths(data_inicio+this.adjust_timezone, data_fim+this.adjust_timezone) * -1;
        for (let i = 0; i <= amount_months; i++) {
            current_data_inicio = format(addMonths(data_inicio+this.adjust_timezone, i), this.month_format);
            current_data_fim = format(addMonths(data_inicio+this.adjust_timezone, i), this.month_format);
            const {complete_data_inicio, complete_data_fim} = this.completeInicioFimDate(current_data_inicio, current_data_fim);
            list_months.push(
                {
    ...
    completeInicioFimDate(data_inicio, data_fim) {
        complete_data_inicio = format(new Date(data_inicio+'-01'+this.adjust_timezone), this.date_format);
        complete_data_fim = format(lastDayOfMonth(new Date(data_fim+this.adjust_timezone)), this.date_format);
